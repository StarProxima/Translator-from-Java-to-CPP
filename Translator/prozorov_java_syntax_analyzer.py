...
def lab4_analyze_click():
    ...
    if not errorPresent:
        messagebox.showinfo('Успешно', 'Синтаксичский анализатор не обнаружил ошибок')
...
def highlight_line(codetxt4, line_num):
    ...
    codetxt4.tag_config("highlight", background="#b19cd9")

window = Tk()
window.title("Translator Java to CSharp")
...
LR4 = ttk.Frame(window)
LR4.pack(fill='both', expand=True)



# UI cинтаксической анализатор

# Создаем appbar
appbar = tk.Frame(LR4, bg="#b19cd9")
appbar.pack(fill='x')

appbar_label = tk.Label(appbar, text="Синтаксический анализатор Java кода", bg="#b19cd9", font=("Arial", 16))
appbar_label.pack(pady=10)

codetxt4 = st.ScrolledText(LR4)
# codetxt4.place(x=280, y=80, width=450, height=390)
codetxt4.pack(padx=10, pady=10)

codetxt4.tag_configure("highlight", background="red")

btngo = tk.Button(LR4, text="Проверить синтаксис", command=lab4_analyze_click, font=("Arial", 14))
